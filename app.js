...
function handleImageUpload(event, kind) {
  ...
  img.onload = () => {
    if (kind === "product") {
      ...
      removeBgBtn.disabled = !originalProductFile || !window.imglyRemoveBackground;
...
async function removeBackgroundFromProduct() {
  ...
  if (!window.imglyRemoveBackground) {
    statusMsg.textContent = "Background removal biblioteka još nije učitana. Sačekaj par sekundi i probaj opet.";
    return;
  }
  ...
  try {
    ...
    const resultBlob = await window.imglyRemoveBackground(originalProductFile);
    ...
    console.error(error);
    statusMsg.textContent = "Remove background nije uspeo za ovu sliku. Probaj drugi PNG/JPG sa jasnijim proizvodom.";
  } finally {
    removeBgBtn.disabled = !originalProductFile;
...
function getTheme(style) {
  if (style === "bold") {
    return {
      ...
      priceFill: "#fff6f8",
      priceText: "#d90452",
      oldPriceText: "rgba(255,255,255,0.90)",
      oldPriceLine: "#ffffff",
      badgeFill: "#f8ea24",
      badgeText: "#121212"
    };
  ...
  if (style === "dark") {
    return {
      ...
      priceFill: "#ffffff",
      priceText: "#111827",
      oldPriceText: "rgba(255,255,255,0.76)",
      oldPriceLine: "rgba(255,255,255,0.9)",
      badgeFill: "#7c6cf2",
      badgeText: "#ffffff"
    };
  }
...
function drawPriceGroup(price, oldPrice, layout, theme) {
  ...
  if (oldPrice) {
    ...
    ctx.textAlign = "left";
    ctx.textBaseline = "alphabetic";
    ctx.font = `800 ${selectedFormat === "story" ? 42 : selectedFormat === "portrait" ? 36 : 30}px Arial`;
    ctx.fillText(oldPrice, x + 8, layout.oldPriceY);

    const oldWidth = ctx.measureText(oldPrice).width;
    ctx.strokeStyle = theme.oldPriceLine;
    ctx.lineWidth = 5;
    ctx.beginPath();
    ctx.moveTo(x + 4, layout.oldPriceY - 16);
    ctx.lineTo(x + oldWidth + 14, layout.oldPriceY - 16);
    ctx.stroke();
  }
